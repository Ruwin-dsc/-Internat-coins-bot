...
module.exports = {
        ...
        message.channel.send({
            ...
            collector.on('collect', (reaction, user) => {
                ...
                if (reaction.emoji.name === "2️⃣") {
                    ...
                    const role = message.guild.roles.cache.get(guildManager.mineZoneRole);
 ...
 getMineInv = function (message, managers, mineZoneManager) {
    ...
    return {
        ...
        "description": `${inv}
                    \`\`\`Achats\`\`\`
                    \`1️⃣\` => Acheter \`x1\` ${managers.ores[managers.ores.length-2].emoji} (${managers.ores[managers.ores.length-2].price} €)
                    \`2️⃣\` => Acheter \`Pioche en bois\` (${managers.ores[managers.ores.length-1].price} €)\n`,
        "color": 3355443,
        "footer": {
            "text": `Emplacement disponible: ${slot} / ${mineZoneManager.slotInventory}`
        },
        "thumbnail": {
